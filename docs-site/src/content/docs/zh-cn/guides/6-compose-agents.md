...
需要把**大段资料**或**生成产物**在 Agent 之间传递时，不要塞到 prompt 里——用 [AFS（跨 Agent 文件共享）](/nap/zh-cn/concepts/afs/)，把文件写到共享目录后授权给协作方，对方在自己的容器里以同一个路径直接读到。
...
完整的设计动机和工作原理见 [Teamwork 概念页](/nap/zh-cn/concepts/teamwork/)。
...
到这里 Agent 的"能力面"和"协作面"都有了。最后一章讲怎么把这些能力**复用、共享、规模化** —— [规模化运营](/nap/zh-cn/guides/7-operate-at-scale/)。
