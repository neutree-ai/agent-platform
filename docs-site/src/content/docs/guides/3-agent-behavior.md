...
Open the **Settings** app in the Workspace (it's in the middle column by default; `⌘K` → **Settings** works from anywhere). The left navigation has 7 areas:

| Area | Purpose |
|---|---|
| General | Workspace basics and lifecycle (start / stop / restart) |
| Model | Choose the agent type, provider, and models |
| Prompt | Write the system Prompt (the most important item) |
| MCP | Connect external tool services; platform capabilities such as Builder Mode also live here |
| Skills | Select which skills to enable |
| Resources | CPU / memory / storage for the Agent container |
| Agent settings | Advanced runtime parameters of the Agent engine |

Memory has its own **Memory** app — it's not inside Settings.
...
- **Model** — a specific model the chosen provider serves
...
Open the **Skills** area in Settings to see the list of all available skills. Select the ones you want to enable and save. They take effect after the Agent restarts.
...
- The **Memory** app on the home screen (press `⌘K` outside any Workspace and open **Memory**) — the account-level view: all your memory stores, creating stores, editing entries, version history
- The **Memory** app inside a Workspace — the current Workspace view: which stores are attached, attach or detach them
...
- **Agent settings** — advanced parameters of the Agent engine itself (Claude Code writes to `.claude/settings.json`, Codex appends to `~/.codex/config.toml`). When opened, the right side shows the field descriptions for the current Agent type
...
Two capabilities — enable either or both; leaving both unchecked keeps Builder Mode off:
...
| **Account-wide** | Cross-Workspace resources under your account | Credentials, providers, the Prompt library, and shares |

With either capability enabled, the Agent automatically gains a set of **read-only** capabilities to use as the basis for its proposals:
...
The capability checkboxes control **what can be written**; the read layer is shared — the Agent can "see the current state clearly before proposing" whichever you enable.
