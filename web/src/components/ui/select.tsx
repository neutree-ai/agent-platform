...
interface SelectItemExtraProps {
  /**
   * Optional secondary text rendered beneath the item label inside the
   * dropdown. Lives outside `ItemText` so it does NOT mirror into the
   * trigger when this item is selected — only the label does.
   */
...
>(({ className, children, description, ...props }, ref) => (
  <SelectPrimitive.Item
    ref={ref}
    className={cn(
      "relative flex w-full cursor-default select-none flex-col items-stretch rounded-sm py-1.5 pl-2 pr-8 text-sm outline-none focus:bg-foreground/[0.06] focus:text-foreground data-[disabled]:pointer-events-none data-[disabled]:opacity-50",
      className,
    )}
    {...props}
  >
    ...
    {description && <span className="mt-0.5 text-tiny text-muted-foreground">{description}</span>}
  </SelectPrimitive.Item>
));
