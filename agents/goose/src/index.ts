...
// --with-builtin developer,summon: the session's extension set is these
// CLI-pinned builtins, then every extension config.yaml leaves enabled, then
// session/new's mcpServers (initial_session_extensions in goose's ACP
// server). Pinning here keeps the two the platform depends on regardless of
// config.yaml or agent_settings. developer is the shell/edit tools; summon
// adds the delegate/load subagent tools (in-process platform extension —
// subagents run as child sessions inside the same goose process, inheriting
// the parent's extensions and provider). What must stay off is listed in
// config.ts.
//
