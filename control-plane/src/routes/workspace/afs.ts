...
const afs = new Hono<WorkspaceAppEnv>()

// afs-fuse sidecar boot pull: list of AFS shares this ws should mount.
//
// The entries carry share access keys, so this is workspace-scoped like the
// credentials route: the sidecar presents the workspace token its runner
// delivered, and requireWorkspaceParam pins the list to that workspace.
...
afs.get('/v1/workspaces/:id/afs-mounts', requireWorkspaceParam(), async (c) => {
