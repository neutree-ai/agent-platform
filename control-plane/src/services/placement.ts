...
// and the env-runner converges actual → desired. For v1 every workspace is on
...
/**
 * Place a freshly-created workspace: desired=running, spec from its config,
 * spec_version=1 with observed_version=0 so the runner applies (creates the pod)
 * on its next pass. Idempotent — a re-create bumps the spec instead.
 */
export async function placeWorkspace(workspaceId: string): Promise<void> {
  ...
  await pool.query(
    ...
    [workspaceId, BUILTIN_ENV, JSON.stringify(spec)],
  )
}
